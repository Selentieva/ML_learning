...
def preprocess_data(raw_df: pd.DataFrame, scaler_numeric: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str], StandardScaler, OneHotEncoder]:
    ...
    return X_train, train_targets, X_val, val_targets, features, scaler, encoder


def preprocess_new_data(new_df: pd.DataFrame, input_cols: List[str], numeric_features: List[str], categorical_features: List[str], scaler: StandardScaler, encoder: OneHotEncoder) -> np.ndarray:
    """
    ...
    - numeric_features (List[str]): List of numeric feature names.
    - categorical_features (List[str]): List of categorical feature names.
    - scaler (StandardScaler): Scaler used for numeric features.
    - encoder (OneHotEncoder): Encoder used for categorical features.
